...
class Encoder(nn.Module):
    ...
    def forward(self, x, relation, mask=None, q_len=None):
        "Pass the input (and mask) through each layer in turn."
        for layer in self.layers:
            x = layer(x, relation, mask, q_len=q_len)
        return self.norm(x)
...
# Adapted from The Annotated Transformer
class CoSublayerConnection(nn.Module):
    """
    A residual connection followed by a layer norm.
    Note for code simplicity the norm is first as opposed to last.
    """
    def __init__(self, size, dropout):
        super(CoSublayerConnection, self).__init__()
        self.norm = nn.LayerNorm(size)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x, y, sublayer):
        "Apply residual connection to any sublayer with the same size."
        return x + self.dropout(sublayer(self.norm(x),self.norm(y)))
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, size, self_attn, feed_forward, num_relation_kinds, dropout, co_attn=None):
        super(EncoderLayer, self).__init__()
        self.self_attn = self_attn
        self.feed_forward = feed_forward
        self.size = size
        self.co_attn = co_attn
        if self.co_attn is not None:
            self.cosublayer = clones(lambda: CoSublayerConnection(size, dropout), 2)
        # else:
        #     self.layers = 2
    ...
    def forward(self, x, relation, mask, q_len=None):
        ...
        if self.co_attn is not None:
            relation_qs_k = relation_k[:q_len, q_len:]
            relation_qs_v = relation_v[:q_len, q_len:]
            relation_sq_k = relation_k[q_len:, :q_len]
            relation_sq_v = relation_v[q_len:, :q_len]

            x_new = x.clone()
            q_enc = x_new[:, :q_len]
            sch_enc = x_new[:, q_len:]
            q_enc = self.cosublayer[0](q_enc, sch_enc, lambda q, s:self.self_attn(q,s,s,relation_qs_k,relation_qs_v) )
            sch_enc = self.cosublayer[1](sch_enc, q_enc, lambda s, q:self.self_attn(s,q,q,relation_sq_k,relation_sq_v) )
            x = torch.cat((q_enc,sch_enc),dim=1)
            # q_enc = self.co_attn(q_enc, sch_enc, sch_enc, relation_sc_k, relation_sc_v, mask)
            # x[:,:q_len] = q_enc
            # sch_enc = slef.co_attn(q_enc, sch_enc, sch_enc, relation_sc_k, relation_sc_v, mask)
            # x_new = x.clone()
            # q_enc = x_new[:,:q_len]
            # sch_enc = x_new[:,q_len:]
