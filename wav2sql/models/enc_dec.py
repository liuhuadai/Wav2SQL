...
class EncDecModel(torch.nn.Module):
    ...
    def _compute_loss_enc_batched(self, batch, debug=False):
        losses = []
        enc_states, cel_loss = self.encoder([enc_input for enc_input, dec_output in batch])
        ...
        if debug:
            ...
            return torch.mean(torch.stack(losses, dim=0), dim=0) + cel_loss, cel_loss
    ...
    def eval_on_batch(self, batch):
        mean_loss = self.compute_loss(batch)[0].item()
