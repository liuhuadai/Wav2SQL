...
from wav2sql import ast_util
# noinspection PyUnresolvedReferences
from wav2sql import datasets
# noinspection PyUnresolvedReferences
from wav2sql import grammars
# noinspection PyUnresolvedReferences
from wav2sql import models
# noinspection PyUnresolvedReferences
from wav2sql import optimizers

from wav2sql.utils import registry
from wav2sql.utils import random_state
from wav2sql.utils import saver as saver_mod
...
@attr.s
class TrainConfig:
    eval_every_n = attr.ib(default=1000)
...
class Trainer:
    def __init__(self, logger, config, tensor_dir):
        if torch.cuda.is_available():
            self.device = torch.device('cuda')
        else:
            self.device = torch.device('cpu')
        self.writer = SummaryWriter(tensor_dir)
        self.logger = logger
    ...
    def train(self, config, modeldir, train_sec='train', dev_sec='dev'):
        ...
        with self.data_random:
            train_data = self.model_preproc.dataset(train_sec)
        ...
        val_data = self.model_preproc.dataset(dev_sec)
                ...
                if last_step % self.train_config.eval_every_n == 0:
                    ...
                    if self.train_config.eval_on_val:
                        eval_loss = self._eval_model(self.logger, self.model, last_step, val_data_loader, 'val',
                                         num_eval_items=self.train_config.num_eval_items)
                        self.writer.add_scalar('eval_loss', eval_loss, last_step)

                # Compute and apply gradient
                with self.model_random:
                    for _i in range(self.train_config.num_batch_accumulated):
                        if _i > 0:  batch = next(train_data_loader)
                        loss, cel_loss = self.model.compute_loss(batch)
                        norm_loss = loss / self.train_config.num_batch_accumulated
                        norm_loss.backward()
                    self.writer.add_scalar('train loss', loss.item(), last_step)
                    # self.writer.add_scalar('cel loss',last_step, cel_loss.item())
...
def main(args):
    ...
    trainer = Trainer(logger, config, tensor_dir=args.logdir)
    trainer.train(config, modeldir=args.logdir, train_sec=args.train_sec, dev_sec=args.dev_sec)
