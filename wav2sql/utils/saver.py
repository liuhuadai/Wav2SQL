...
def load_checkpoint(item_dict, model_dir, map_location=None, step=None):
    ...
    if os.path.exists(path):
        ...
        # print(item_dict['optimizer'].keys())
        for item_name in item_dict:
            if item_name == 'model':item_dict[item_name].load_state_dict(checkpoint[item_name], strict=False)
            else: item_dict[item_name].load_state_dict(checkpoint[item_name])
